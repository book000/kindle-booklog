...
- `puppeteer-core`（ブラウザ自動化。HTTP は page 経由で行い、専用 HTTP クライアントは使わない）、`csv-parse`（ブクログの CSV）、`iconv-lite`（CSV 文字コード変換）、`otplib`（OTP）、`tar-stream`（Kindle レスポンスの tar 展開）、`@book000/node-utils`（Logger / Discord）
